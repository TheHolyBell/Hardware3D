...
float ShadowLoop_(const in float3 spos, uniform int range)
{
    ...
    for (int x = -range; x <= range; x++)
    {
        ...
        for (int y = -range; y <= range; y++)
        {
            if (hwPcf)
            {
                shadowLevel += g_ShadowMap.SampleCmpLevelZero(ssamHw, spos.xy, spos.b - depthBias, int2(x, y));
            }
            else
            {
                shadowLevel += g_ShadowMap.Sample(ssamSw, spos.xy, int2(x, y)).r >= spos.b - depthBias ? 1.0f : 0.0f;
            }
        }
    }
