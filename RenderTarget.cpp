...
namespace Bind
{
		...
		m_Height(height)
	{
		...
		GFX_THROW_INFO(GetDevice(gfx)->CreateRenderTargetView(
			pTexture.Get(), &rtvDesc, &m_pTargetView
		));
	}
	...
	RenderTarget::RenderTarget(Graphics& gfx, ID3D11Texture2D* pTexture)
	{
		...
		rtvDesc.Texture2D = D3D11_TEX2D_RTV{ 0 };
		GFX_THROW_INFO(GetDevice(gfx)->CreateRenderTargetView(
			pTexture, &rtvDesc, &m_pTargetView
		));
	}
	...
	{
		BindAsBuffer(gfx, depthStencil ? depthStencil->m_pDepthStencilView.Get() : nullptr);
	}

	void RenderTarget::BindAsBuffer(Graphics& gfx, ID3D11DepthStencilView* pDepthStencilView) noxnd
	{
		INFOMAN_NOHR(gfx);
		GFX_THROW_INFO_ONLY(GetContext(gfx)->OMSetRenderTargets(1, m_pTargetView.GetAddressOf(), pDepthStencilView));
	...
	{
		INFOMAN_NOHR(gfx);
		GFX_THROW_INFO_ONLY(GetContext(gfx)->ClearRenderTargetView(m_pTargetView.Get(), color.data()));
	}
		...
		m_Slot(slot)
	{
		...
		m_pTargetView->GetResource(&pRes);
	...
	Surface Bind::ShaderInputRenderTarget::ToSurface(Graphics& gfx) const
	{
		...
		m_pShaderResourceView->GetResource(&pResSource);
