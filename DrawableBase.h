...
private:
	virtual const std::vector<std::unique_ptr<Bind::Bindable>>& GetStaticBinds() const noexcept override
	{
		return s_StaticBinds;
	}
private:
	static std::vector<std::unique_ptr<Bind::Bindable>> s_StaticBinds;
};

template<typename T>
std::vector<std::unique_ptr<Bind::Bindable>> DrawableBase<T>::s_StaticBinds;