...
{
public:
	static IndexedTriangleList MakeTesselated(Dynamic::VertexLayout layout, int latDiv, int longDiv)
	{
		...
		Dynamic::VertexBuffer vb{ std::move(layout) };
	...
	static IndexedTriangleList Make(std::optional<Dynamic::VertexLayout> layout = std::nullopt)
	{
		using Element = Dynamic::VertexLayout::ElementType;
		if (!layout)
		{
			layout = Dynamic::VertexLayout{}.Append(Element::Position3D);
		}
